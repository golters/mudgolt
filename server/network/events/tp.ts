...
const handler: NetworkEventHandler = async (socket, roomNameInput: string, player) => {
  try {
    ...
    await insertRoomCommand(oldRoom.id, player.id, `${player.username} has teleported from ${oldRoom.name}`, Date.now(), "tp")
    ...
    await insertRoomCommand(room.id, player.id, `${player.username} has teleported into ${room.name}`, Date.now(), "tp")
