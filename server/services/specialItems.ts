export interface itemtypes {
  name: string
  bio: string
  icon: string
  rarity: string
}

export const fish: itemtypes[] = [
  {
    name:"trout",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"carp",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"bass",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"cod",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"herring",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"eel",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"tuna",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"haddock",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"fish",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"shark",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"dolphin",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"whale",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"crab",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"lobster",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"shrimp",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"squid",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"octopus",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"mermaid",
    bio:"a fish with the body of a human",
    icon: "123456123456123456",
    rarity: "1",
  },{
    name:"ray",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"clam",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"urchin",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"trilobite",
    bio:"",
    icon: "123456123456123456",
    rarity: "1",
  },{
    name:"turtle",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"pirahna",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },{
    name:"frog",
    bio:"",
    icon: "123456123456123456",
    rarity: "0",
  },
]

export const bugs: itemtypes[] = [
  {
    name:"bug",
    bio:"a little crawly guy with lots of legs",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  }, {
    name:"bug",
    bio:"it looks like a regular item, but it is infact a bug",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "1",
  },{
    name:"roach",
    bio:"a flat round bug",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"mantis",
    bio:"a tall bug with wing like claws",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "1",
  },{
    name:"slug",
    bio:"a slow slimy blob",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"snail",
    bio:"a little slimy boy who lives in a spiral shell house",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"worm",
    bio:"a wriggly boy with no legs",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"butterfly",
    bio:"a beautiful four winged bug with a thin body and vibrant colors",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"moths",
    bio:  "a flufy flying bug with scaly wings",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"spider",
    bio:"an eight legged little guy who lives on a web of silk",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"scorpion",
    bio:"a long insect with big claws and a curled stinger tail",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"fly",
    bio:"a small flying bug with shiny glass like wings",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"bee",
    bio:"a fuzzy yellow and black flying insect",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"locust",
    bio:"a long green insect with huge spring like back legs",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"beetle",
    bio:"a round shelled bug with little horns",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"arthropleura",
    bio:"a giant insect with hundreds of legs. thought to be long extinct",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "1",
  },{
    name:"caterpilla",
    bio:"a hungry little guy",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"cocoon",
    bio:"a hard shell containing a little bug as it grows into it's next form",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"pupae",
    bio:"a soft and squishy baby bug",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "0",
  },{
    name:"tardigrade",
    bio:"a little microanimal. he's so so very small",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "1",
  },{
    name:"fairy",
    bio:"a tiny person with insect wings",
    icon: "∙∙୧୨∙∙∙\◠∙/∙∙/◡∙\∙",
    rarity: "2",
  },
]

export const weeds: itemtypes[] = [
  {
    name:"weed",
    bio:"a tenacious little plant that seems to be able to grow anywhere",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"thistle",
    bio:"a sharp prickly stem holding up a purple firework like flower",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"daisy",
    bio:"a cut little white flower",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"clover",
    bio:"a tiny three leafed plant",
    icon: "∙∙∙∙∙∙∙∙☘∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"clover",
    bio:"a tiny four leafed plant, lucky you",
    icon: "∙∙∙∙∙∙∙∙☘∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"grass",
    bio:"a small green pointy plant",
    icon: "∙∙∙∙∙∙∙∙∙∙∙∙∙෴∙∙∙∙",
    rarity: "0",
  },{
    name:"dandelion",
    bio:"a fluffy plant that drops little floaty seeds everywhere, make a wish",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"garlic",
    bio:"a bouquet of tiny white flowers sprouting from the ground, perhaps a tasty treat lies beneath",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"carrot",
    bio:"a tight bunch of white flowers sprouting from a crunchy wild carrot",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"mushroom",
    bio:"a white rubbery cap, maybe it's edible, might be poisonous",
    icon: "1∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"lichen",
    bio:"a network of microscopic plants forming a crusty goop",
    icon: "∙∙∙∙∙∙∙∙∙∙∙∙∙෴∙∙∙∙",
    rarity: "0",
  },{
    name:"moss",
    bio:"a tightly woven clump of tiny grass like plants",
    icon: "∙∙∙∙∙∙∙∙∙∙∙∙∙෴∙∙∙∙",
    rarity: "0",
  },{
    name:"flower",
    bio:"a pretty flower",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"bamboo",
    bio:"a tall hard wood like stick",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"reeds",
    bio:"very tall grass, tall enough to hide in",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"nettle",
    bio:"a fuzzy looking plant thats sharp and stings",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"cactus",
    bio:"rows of spikes protect a juicy green plant",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"succulent",
    bio:"it looks like a flower but it's green and juicy like a fruit",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"orchid",
    bio:"a beautiful delicate flower",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "1",
  },{
    name:"ivy",
    bio:"spirals of wirey plant vines covered in three pointed leafs",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"vines",
    bio:"ropey plant fibers that crawl along the ground",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"bell",
    bio:"a delicate flower with bell shaped petals",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"acorn",
    bio:"a small hardy seed with a bumpy hat",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"pinecone",
    bio:"an intricate spiral of wooden tongues",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"nightshade",
    bio:"a deadly poisonous plant with black berries and star shaped leaves",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"bean",
    bio:"a long pod of beans, good for your heart, the more you eat the more you fart",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"nut",
    bio:"a spikey green ball",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"drosera",
    bio:"a sticky carnivorous plant",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"lotus",
    bio:"a pink mandala shaped water flower",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"hibiscus",
    bio:"a beautiful spiral flower with a long polen coated antena",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"rafflesia",
    bio:"a huge parasitic flower",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"truffle",
    bio:"a tasty underground mushroom",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "1",
  },{
    name:"bramble",
    bio:"a bush of thorny vines",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"shrubbery",
    bio:"a well groomed bush",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "0",
  },{
    name:"mound",
    bio:"a shambling sentient mass of roots and vines",
    icon: "∙∙∙∙∙∙∙☙✿∙∙∙∙∙↡∙∙∙",
    rarity: "2",
  },
]
